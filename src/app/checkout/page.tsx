...
export default function CheckoutPage() {
  const { lines, selectedShipping, destino, clearCart } = useCart();
  ...
  if (lines.length === 0) {
    return (
      <div className="container-nb py-28 text-center">
        <p className="text-nb-stone">Tu carrito está vacío.</p>
        <Link href="/productos" className="text-nb-wood hover:text-nb-roseDeep">
          Ver colección →
        </Link>
      </div>
    );
  ...
  return (
    <div className="container-nb py-14">
      <h1 className="font-serif text-3xl text-nb-black mb-10">Checkout</h1>
      ...
      <div className="grid lg:grid-cols-[1fr_380px] gap-14">
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid sm:grid-cols-2 gap-4">
            {FIELDS.map((f) => (
              <div key={f.name} className={f.span === 2 ? "sm:col-span-2" : ""}>
                <label className="block text-xs tracking-wide text-nb-taupe mb-1.5">
                  {f.label}
                  {f.required && " *"}
                </label>
                <input
                  ...
                  className="w-full px-3.5 py-2.5 border border-nb-black/20 bg-white text-sm focus-ring focus:border-nb-wood outline-none"
                />
              </div>
            ))}
          </div>
          ...
          {!selectedShipping && (
            <p className="text-sm text-nb-stone">
              No elegiste un método de envío todavía.{" "}
              <Link href="/carrito" className="text-nb-wood hover:text-nb-roseDeep">
                Volver al carrito
              </Link>
            </p>
          )}

          <div className="border-t border-nb-black/10 pt-5">
            <p className="text-xs tracking-widest2 text-nb-taupe mb-2">MÉTODO DE PAGO</p>
            <p className="text-sm text-nb-stone">
              La integración con Mercado Pago está preparada y pendiente de credenciales.
              Por ahora, tu pedido queda registrado como <strong>pendiente de pago</strong> y
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full py-3.5 bg-nb-black text-nb-bone text-sm tracking-wide hover:bg-nb-ink transition-colors disabled:opacity-50 focus-ring"
          >
            {submitting ? "Generando pedido…" : "CONFIRMAR PEDIDO"}
          </button>
        </form>

        <aside className="border border-nb-black/10 p-6 h-fit">
          <h2 className="font-serif text-xl text-nb-black mb-5">Tu pedido</h2>
          <div className="space-y-3 text-sm">
            {lines.map((l) => {
              const p = products[l.productId];
              if (!p) return null;
              return (
                <div key={`${l.productId}-${l.variantId}`} className="flex justify-between text-nb-stone">
                  <span>
                    {p.nombre} × {l.cantidad}
                  </span>
          ...
          <div className="border-t border-nb-black/10 mt-4 pt-4 space-y-2 text-sm">
          ...
          <div className="border-t border-nb-black/10 mt-3 pt-3 flex justify-between text-nb-black">
            <span>Total</span>
